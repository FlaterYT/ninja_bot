...
client.on('message', async message => {
  ...
  if(message.content == '+fortnite'){
        ...
        request(options, (err, res, body) => {
          if(err){
            console.error(err)
            message.reply(`There was a problem getting stats for ${username.replace('%20', ' ')} on ${platform}.`)
            return null
          }
            ...
            message.reply(`Something went wrong. Sorry :b:ud.`)
            return null
          }
        })}})
